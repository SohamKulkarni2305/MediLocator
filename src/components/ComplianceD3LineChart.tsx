...
export const ComplianceD3LineChart: React.FC<ComplianceD3LineChartProps> = ({
  ...
  const stats = useMemo(() => {
    ...
    const peakDay = filteredData.reduce(
      (max, curr) => (curr.totalThroughput > max.totalThroughput ? curr : max),
      filteredData[0] || { totalThroughput: 0, displayDate: 'N/A' }
    );
    const peakFlagDay = filteredData.reduce(
      (max, curr) => (curr.flaggedCount > max.flaggedCount ? curr : max),
      filteredData[0] || { flaggedCount: 0, displayDate: 'N/A' }
    );
  ...
  }, [filteredData, chartMode, showThroughput, showFlagged, showAreaFill, selectedPoint, onSelectPoint, stats.peakFlagDay]);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1 text-xs">
        ...
        <div className="p-3 bg-gradient-to-b from-slate-50 to-white rounded-xl border border-slate-200/80 shadow-2xs">
          ...
          <div className="text-base sm:text-lg font-headline font-extrabold text-slate-800 mt-1 truncate">
            {stats.peakFlagDay.flaggedCount} Flags
          </div>
          <span className="text-[10px] text-slate-500 font-mono truncate block mt-0.5">
            {stats.peakFlagDay.displayDate} ({stats.peakFlagDay.primaryFlagPattern.split(' ')[0]})
          </span>
        </div>
      </div>
