...
export const AdminConsole: React.FC<AdminConsoleProps> = ({
  ...
  const clusters = ['All clusters', ...Array.from(new Set(kycList.map((pharmacy) => pharmacy.cluster)))];
  const filteredKycList = clusterFilter === 'All clusters'
    ? kycList
    : kycList.filter((pharmacy) => pharmacy.cluster === clusterFilter);
  const activeClusterCount = new Set(
    kycList.filter((pharmacy) => pharmacy.status === 'approved').map((pharmacy) => pharmacy.cluster)
  ).size;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden flex flex-col">
            <div className="p-4 sm:p-5 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <label className="sr-only" htmlFor="cluster-filter">Filter pharmacies by cluster</label>
                <select
                  id="cluster-filter"
                  value={clusterFilter}
                  onChange={(event) => setClusterFilter(event.target.value)}
                  className="max-w-[190px] rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-500"
                  data-testid="admin-cluster-filter"
                >
                ...
                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-900">
                  {filteredKycList.filter(k => k.status !== 'approved').length} Actions Required
                </span>
              </div>
            </div>

            <div className="grid grid-cols-3 gap-2 border-b border-slate-200 bg-white p-3 text-center">
              <div><div className="text-lg font-bold text-slate-900">{filteredKycList.length}</div><div className="text-[10px] uppercase tracking-wider text-slate-500">Nodes in view</div></div>
              <div><div className="text-lg font-bold text-emerald-700">{filteredKycList.filter(k => k.status === 'approved').length}</div><div className="text-[10px] uppercase tracking-wider text-slate-500">Active nodes</div></div>
              <div><div className="text-lg font-bold text-sky-700">{activeClusterCount}</div><div className="text-[10px] uppercase tracking-wider text-slate-500">Active clusters</div></div>
            </div>

            <div className="divide-y divide-slate-200">
              {filteredKycList.map((pharmacy) => (
                <div
                  key={pharmacy.id}
